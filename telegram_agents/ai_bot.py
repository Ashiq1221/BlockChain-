...
def _quick_buttons() -> InlineKeyboardMarkup:
    return InlineKeyboardMarkup([
        ...
        [
            InlineKeyboardButton("🎼 100-Agent Run", callback_data="orchestra"),
            InlineKeyboardButton("⚡ Full Execute",  callback_data="execute_all"),
        ],
    ])
...
class AIBot:
    def __init__(self, bot: Client, user_client: Client, tools: ToolRegistry, db: Database):
        ...
        self.tools       = tools
        self.db          = db
        self.orchestra   = Orchestra(tools, db)
    ...
    async def start(self):
        ...
        self.bot.add_handler(MessageHandler(self._cmd_orchestra, filters.command("orchestra") & filters.private))
        self.bot.add_handler(MessageHandler(self._cmd_orchestra, filters.command("o100")      & filters.private))
        self.bot.add_handler(MessageHandler(self._cmd_orchestra, filters.command("execute")   & filters.private))
        self.bot.add_handler(MessageHandler(self._cmd_debate,    filters.command("debate")    & filters.private))
        # Any text
        ...
        console.print(f"[bold green]🤖 Telegram Bot ONLINE[/bold green] — 🎼 {self.orchestra.agent_count()} agents ready")
    ...
    async def _cmd_start(self, bot: Client, msg: Message):
        if not self._owner_only(msg.from_user.id):
            return
    ...
    async def _cmd_orchestra(self, bot: Client, msg: Message):
        if not self._owner_only(msg.from_user.id):
            return
        uid = msg.from_user.id
        task = (msg.text or "").split(None, 1)[1].strip() if len((msg.text or "").split()) > 1 else \
               "full autonomous: discover web3 AI opportunities, analyze top ones, draft outreach, check stats"
        ack = await msg.reply(
            f"🎼 *100-Agent Orchestra launching...*\n\n"
            f"Task: _{task[:80]}_\n\n"
            "Activating agents across 9 departments...",
            parse_mode="markdown"
        )
        async def _progress(update: str):
            try:
                await ack.edit(f"🎼 *Orchestra running...*\n\n_{update}_", parse_mode="markdown")
            except Exception:
                pass

        result = await self.orchestra.run(task, progress_cb=_progress)
        try:
            await ack.edit(result, reply_markup=_quick_buttons(), parse_mode="markdown")
        except Exception:
            await bot.send_message(uid, result, reply_markup=_quick_buttons(), parse_mode="markdown")

    async def _cmd_debate(self, bot: Client, msg: Message):
        if not self._owner_only(msg.from_user.id):
            return
        from telegram_agents.tools.ai_router import think_debate
        question = (msg.text or "").split(None, 1)[1].strip() if len((msg.text or "").split()) > 1 else ""
        if not question:
            await msg.reply(
                "🤝 *AI Debate Mode*\n\n"
                "All 5 AI providers answer simultaneously, then a synthesizer picks the best.\n\n"
        ...
        try:
            await ack.edit(f"🏆 *Debate Synthesized:*\n\n{result}", parse_mode="markdown")
        except Exception:
            await bot.send_message(msg.from_user.id, f"🏆 *Debate Result:*\n\n{result}", parse_mode="markdown")

    # ── Button callbacks ─────────────────────────────────────────────────────────

    async def _on_button(self, bot: Client, cb: CallbackQuery):
        if not self._owner_only(cb.from_user.id):
            await cb.answer()
            return
        await cb.answer()
        mapping = {
            ...
            "orchestra":   "full autonomous: discover web3 AI opportunities, analyze top ones, draft outreach",
            "execute_all": "execute full pipeline: discover projects, join groups, research founders, send DMs, report stats",
        }
        task = mapping.get(cb.data, cb.data)

        # Orchestra button runs the 100-agent system
        if cb.data in ("orchestra", "execute_all"):
            uid = cb.from_user.id
            ack = await bot.send_message(uid, f"🎼 *100-Agent Orchestra launching...*\n\n_{task[:80]}_\n\nActivating all departments...", parse_mode="markdown")
            async def _prog(u: str):
                try:
                    await ack.edit(f"🎼 *Orchestra running...*\n\n_{u}_", parse_mode="markdown")
                except Exception:
                    pass
            result = await self.orchestra.run(task, progress_cb=_prog)
            try:
                await ack.edit(result, reply_markup=_quick_buttons(), parse_mode="markdown")
            except Exception:
                await bot.send_message(uid, result, reply_markup=_quick_buttons())
            return
        uid = cb.from_user.id
        ack = await bot.send_message(uid, f"⏳ *{task[:60]}*\n\nPlanning...")
    ...
    async def _run_task(self, uid: int, task: str, ack: Message | None = None) -> str:
        ...
        try:
            ...
            from telegram_agents.brain import _json_array
            steps = _json_array(plan_raw)
            if not steps:
    ...
    async def _bulk_join_groups(self, topic: str, count: int = 5) -> dict:
        ...
        if not groups:
            return {"ok": False, "error": f"No Telegram groups found for '{topic}' via web search"}
        joined = 0
        errors = []
        from telegram_agents.tools import telegram_tools
        for g in groups[:count]:
            ...
            try:
                ...
                if ok:
                    ...
                    errors.append(f"@{username}: join returned False")
                await asyncio.sleep(4)
            except Exception as e:
                errors.append(f"@{username}: {e}")
        detail = f" | Errors: {'; '.join(errors[:3])}" if errors else ""
        return {"ok": joined > 0, "result": f"Joined {joined}/{min(count, len(groups))} groups about '{topic}'{detail}"}
